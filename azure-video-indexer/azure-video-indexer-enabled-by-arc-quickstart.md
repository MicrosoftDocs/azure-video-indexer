...
The below is a block diagram showing Azure Video Indexer running on Azure Arc. There are types: Store type A uses both vision and audio presets while Stor type B uses only vision presets. It also has a custom model. For more information about using a custom model with Azure Video Indexer enabled by Arc, see [Bring Your Own AI model](azure-video-indexer-enabled-by-arc-bring-your-own-model-overview.md). Store C uses only audio presets. The extension is stored on the edge device, each associated with an AI Video Indexer account which interfaces with Azure Arc and the cloud.
